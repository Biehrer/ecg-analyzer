...
//! Interface for MIT-ECG Data
//!
//! Usage:
//! - Set the wdbf path to the directory where your record files are located via SetWDFBPath(database_path)
//! - Use Read(record_name) with the record name as argument
//!
...
struct MITDataChannel_TP {
    ...
    int _adc_baseline_0mV_output_U = 0;
    // adc output in volts when 0 physical units used as input
    int _adc_baseline_0U_output_mV = 0;
    //! number of data samples
    unsigned int _num_samples = 0;
    //! adc resolution in bits
    int _adc_resolution_bits = 0;
...
public:
    ...
    //~MITFileIO_C();
public:
    const std::vector<MITDataChannel_TP<SampleDataType_TP>> Read(char* record_name);
...
MITFileIO_C<SampleDataType_TP>::Read(char* record_path)
{
    ...
    for (int channel_count = 0; channel_count < number_of_signals; ++channel_count ) {
        ...
        // number of samples
        channel_data[channel_count]._num_samples = static_cast<unsigned int>(signal_info[channel_count].nsamp);
        // mv or other 
    ...
    for ( int sample_count = 0; sample_count < number_of_samples; ++sample_count ) {
        ...
        // insert the sample of each channel into its corresponding data vector
